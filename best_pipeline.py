...
class FM:
    def __init__(self, dim, k=16, lr=0.001, l2=1e-6, seed=0, temp=1.0, recency_scale=1.0, margin_weight=0.1):
        rng = np.random.default_rng(seed)
        self.V = rng.normal(0, 0.01, (dim, k)).astype(np.float32)
        self.W = np.zeros(dim, dtype=np.float32)
        self.b = np.float32(0.0)
        self.lr, self.l2, self.temp, self.recency_scale, self.margin_weight = lr, l2, temp, recency_scale, margin_weight
    ...
    def step(self, X, y, sample_weights):
        B = len(y)
        z, E, S = self.logits(X)
        z_clipped = np.clip(z / self.temp, -3.0, 3.0)  # temperature scaling on logits before sigmoid
        pred = sigmoid(z_clipped)
        eps = 0.05
        y_smoothed = y * (1.0 - eps) + eps
        # Margin-aware gradient reweighting: scale by (1 + λ * y * z)
        # Use original z (pre-clipped) for margin to preserve scale
        margin = y * z
    ...
    def predict(self, X, bs=200_000):
        ...
        for i in range(0, len(X), bs):
            ...
            z_clipped = np.clip(z / self.temp, -3.0, 3.0)
            preds.append(sigmoid(z_clipped))
        return np.concatenate(preds)

def run_fm(splits, k=16, lr=0.001, epochs=40, bs=8192, patience=4, seed=0, verbose=True, recency_scale=1.0, margin_weight=0.1):
    enc, dim = encode(splits)
    Xtr, ytr, _ = enc['train']; Xva, yva, uva = enc['valid']; Xte, yte, ute = enc['test']
    
    # Precompute recency weights: linear decay over training set, latest samples get highest weight
    N = len(ytr)
    ...
    m = FM(dim, k=k, lr=lr, seed=seed, temp=0.85, recency_scale=recency_scale, margin_weight=margin_weight)
    rng = np.random.default_rng(seed)
...
if __name__ == "__main__":
    ...
    res = run_fm(splits, k=16, lr=0.001, epochs=40, seed=0, verbose=False, recency_scale=0.5, margin_weight=0.2)
